...
/**
 * Base API client for SAP CIAM (Customer Data Cloud) REST API operations.
 * 
 * This class provides the core HTTP communication layer for interacting with SAP CIAM services.
 * It handles network connectivity checks, HTTP GET/POST requests, server time synchronization,
 * and error handling for all CIAM API operations.
 * 
 * The Api class:
 ...
 * - Converts HTTP responses to CIAMResponse objects
...
open class Api(private val coreClient: CoreClient) {
    ...
    /**
     * Performs a generic HTTP GET request to a CIAM API endpoint.
     * 
     ...
     * - Converts the HTTP response to a CIAMResponse object
     * - Handles HTTP exceptions and network errors
     * 
     * @param request The CIAMRequest containing the API endpoint, parameters, and headers
     * @return CIAMResponse containing the parsed response data or error information
     * 
     * @see CIAMRequest
     * @see CIAMResponse
     * @see networkAvailable
     */
    open suspend fun get(request: CIAMRequest): CIAMResponse {
        if (!networkAvailable()) {
            // Propagate network error.
            return CIAMResponse().noNetwork()
        }
        ...
        return try {
            ...
            CIAMResponse().fromJSON(result.body())
        } catch (e: HttpExceptions) {
            CIAMResponse().fromHttpException(e)
        }
    }

    /**
     * Performs a generic HTTP POST request to a CIAM API endpoint.
     * 
     ...
     * @see networkAvailable
     */
    open suspend fun post(request: CIAMRequest): CIAMResponse {
        if (!networkAvailable()) {
            // Propagate network error.
            return CIAMResponse().noNetwork()
        }
    ...
    /**
     * Sends a generic HTTP request with the specified method to a CIAM API endpoint.
     * 
     * This is a convenience method that routes requests to either [get] or [post] based
     * on the specified HTTP method. It provides a unified interface for making CIAM API
    ...
    suspend fun send(
        ...
        return try {
            ...
            return CIAMResponse().fromHttpException(e)
        }
    }
